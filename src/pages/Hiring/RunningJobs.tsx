...
const RunningJobs = () => {
  ...
  // const { data, isLoading, isFetching } = useQuery('get-all-beneficiaries', () => getAllBeneficiary('phone'));


  // const filterBen = (data: BenTypes[]) => {
  //     return (
  //         data?.filter((item: BenTypes) => item?.info?.name.toLowerCase().includes(search.toLowerCase())
  //             || item?.info?.name.toLowerCase().includes(search.toLowerCase())
  //             || item?.info?.bankName?.toLowerCase().includes(search.toLowerCase())
  //             || item?.info?.accountNumber?.toLowerCase().includes(search.toLowerCase())
  //         )
  //     )
  // }
  // const NoData = () => dataCards.length === 0 || dataCards.filter((item) => item?.staff?.toLowerCase().includes(search.toLowerCase())).length === 0 && <EmptyData title="Education History is Empty" subtitle="Education history will show here when you save them" />
  ...
  return (
    <>
      <div className="mb-3">
        <Input
          value={search}
          onChange={(e: ChangeEvent<HTMLInputElement>) => setSearch((e.target as HTMLInputElement).value)}
          LeadingIcon={() => <>
          ...
          className="!dark:bg-[#232121] dark:text-[#000000]"
